...
export function LogoLoading({
  ...
  // If external progress is provided, use it, otherwise animate automatically
  const currentProgress = progress !== undefined ? progress : animProgress;

  // Initial setup - measure path length once
  useEffect(() => {
    if (!svgRef.current || !outlinePathRef.current || initialized) return;
    try {
      const pathElement = outlinePathRef.current;
      const actualLength = pathElement.getTotalLength();
      setPathLength(actualLength);
      // Set initial styling
      pathElement.setAttribute("fill", "none");
      pathElement.setAttribute("stroke", "currentColor");
      pathElement.setAttribute("stroke-width", full ? "4" : "2");
      pathElement.setAttribute("stroke-dasharray", `${actualLength}`);
      pathElement.setAttribute("stroke-dashoffset", `${actualLength}`);
  ...
  // Handle animation
  useEffect(() => {
    if (progress !== undefined || !pathLength || !initialized) return;
  ...
  }, [duration, progress, repeat, pathLength, initialized]);

  // Update stroke dashoffset based on progress
  useEffect(() => {
    if (!outlinePathRef.current || !pathLength || !initialized) return;

    // Calculate dashOffset
    const dashOffset = pathLength * (1 - currentProgress / 100);
    outlinePathRef.current.style.strokeDashoffset = `${dashOffset}`;
